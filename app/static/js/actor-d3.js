$(function(){
  $.get('/actor/graph', {id: 'Robin', password: '123456'}, function(result) {
...
var varCountButtons = d3.selectAll('#actor-pane .var-count button'),
    varTable = d3.select('#actor-pane table.propvars'),
    varTableBody = d3.select('#actor-pane table.propvars tbody'),
    varSubmmit = d3.select('#model-submmit'),
    varTableRows = varTable.selectAll('tr'),
    varAvatarRow = d3.select('#avatar-content'),
    selectedNodeLabel = d3.select('#actor-pane .selected-node-id'),
...
function mousedown() {
  ...
  var point = d3.mouse(this),
      node = {"name": " 请输入", "weight":1,"gender":" 请输入","country":" 请输入","birthday":" 请输入","job":" 请输入"};
  node.x = point[0];
  node.y = point[1];
  nodes.push(node);
